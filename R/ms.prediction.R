...
utilities.of.profiles <- function(profiles, ratings, bundles) {
...
################### other variables in the function ##################
  ...
  client.sim <- predict(main.effects.model.fit, newdata = profiles)  #now we use the model to predict the utility that each product profile offers to the respondent
  ...
  for (client in 2:clients) {
    ...
    client.sim <- predict(main.effects.model.fit, newdata = profiles)
...
ms.fe.conjoint <- function(profiles, ratings, bundles) {
  #  Copyright 2016 Jordi L. Sintas
  #  This program is free software; you can redistribute it and/or
  #  modify it under the terms of the GNU General Public License
  #  as published by the Free Software Foundation; either version 2
  ...
  ############################
  up <- utilities.of.profiles(profiles, ratings, bundles)  #gets the matrix of utilities using the function utilitiesProfiles()
...
ms.us.conjoint <- function(profiles, ratings, bundles) {
  ...
  #' is the matrix of profiles for which we are going to compute market
  #' profiles is a matrix of product profiles in the market (rows)
  #' by attributes (columns) ratings is a matrix of clients (rows) by
  #' bundles rated (colums) conjointBundles is a matrix of bundles (rows)
  #' by attributes (colums)
  #' @param profiles         a matrix with the description of competitors' product profiles
  #' @param ratings                 a data frame with all clients' ratings
  #' @param bundles                 a data frame with all product profiles rated by clients
  #' @return ms
  #' @export
  ...
  ############################ other variables in the function #########
  # up                      the results of the utilities.of.profiles() function
  # sum.util.client        Sum rows across respondents
  # profiles.prob.client   compute de probability of buying each brand
  # ms                     for each brand compute the mean across respondents
  # brand.name             collects brands' names
  #############################
  up <- utilities.of.profiles(profiles, ratings, bundles)  #gets the matrix of utilities using the function utilitiesProfiles()
...
ms.logit.conjoint <- function(profiles, ratings, bundles) {
  ...
  # up                      the results of the utilities.of.profiles() function
  # exp.up                  exp(U)
  # sum.exp.util.client    Sum rows across respondents
  # profiles.prob.client   compute de probability of buying each brand
  # ms                     for each brand compute the mean across respondents
  # brand.name             collects brands' names

  up <- utilities.of.profiles(profiles, ratings, bundles)
  exp.up <- exp(up)
